...
# function which takes as input sdf file and returns SMILES notation string of the molecule
def find_smiles_in_sdf(sdf_file):
    ...
    smiles_match = re.search(smiles_tag, file_content, re.DOTALL)
...
# this function takes as input SMILES string and returns list of SMILES substrings corresponding to groups (rings or acyclic fragments)
...
def find_ring_size(group):
    ...
    if len(re.findall(r'[A-Z][a-z]?{}'.format(group_index), group)) > 1:
        ...
        ring_group = re.search(r'^[A-Z][a-z]?([0-9]+).*[A-Z][a-z]?\1', group, re.DOTALL).group()
...
def transform_group(group, ring_size):
    ...
    if multiple_subgroups:
        # get first subgroup
        first_subgroup = re.search(r'\(.*?\)', group, re.DOTALL).group()
...
def revert_group_indexation(group):
    ...
    if acyclic_group_found:
        ...
        if (acyclic_group_found > len(group_list) - 1):
            for group in group_list[(acyclic_group_found )::-1]:
                # revert indexation for ring groups
                ring_size = find_ring_size(group)
                if ring_size:
                    group = revert_group_indexation(group)
                # revert group (like initial element is on the other side of the chain)
                group_rev = revert_group(group)
                # add reverted group to the list
...
# this function takes as input SMILES formula of one chemical group and return its SMILES formula so that initial element is on the other side of the chain)
def revert_group(group):
    ...
    if chain_subgroup_match:
        ...
        # get main chain
        group = re.sub(r'\(.*\)', '', group)
        # revert subgroup chain
        subgroup_rev = revert_chain(chain_subgroup, group_index, chain_subgroup_match, subgroup = True)
    # revert main chain
    chain_rev = revert_chain(group, group_index, chain_subgroup_match, subgroup = False)
    # define reverted group as main chain
    group_rev = chain_rev
    # add subgroup chain if exists
    if chain_subgroup_match:
        group_rev += '(' + subgroup_rev + ')'
    
    return group_rev

# this function takes input a part of group (main of subgroup) and returns itself reverted
def revert_chain(chain, group_index, chain_subgroup_match, subgroup):
    element_list = []
...
def main():
    ...
    parser.add_argument('fsmiles_filepath', type = str, help = 'filepath to write fsmiles')
